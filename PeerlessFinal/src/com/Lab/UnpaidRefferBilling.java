...
public class UnpaidRefferBilling extends JPanel{
	...
	String TestCol[]={"S/N","Lab No","Fiscal Year","Patient Name","Age","Sex","Date","Total Charnge","Discout","Payable","Refer Commission","Check"};
	Object TestRow[][]={};
	...
	public UnpaidRefferBilling(SessionBeam sessionBeam) {
...
//Far
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(CheckAll.isSelected()){
					for(int a=0;a<table.getRowCount();a++){
						table.setValueAt(new Boolean(true), a, 11);
					}
				}
				else{
					for(int a=0;a<table.getRowCount();a++){
						table.setValueAt(new Boolean(false), a, 11);
					}
				}
			}
		});
	...
	private void btnFindEvent(){
		//		System.err.println("docor : "+cmbDoctorName.txtMrNo.getText().trim().toString());
		if(!cmbDoctorName.txtMrNo.getText().trim().toString().isEmpty()){
			model.setRowCount(0);
			try {
				int i=1;

				String sql="select * from TbLabPatient where Datetime between '"+new SimpleDateFormat("yyyy-MM-dd").format(txtStartDate.getDate())+"' and '"+new SimpleDateFormat("yyyy-MM-dd").format(txtEndDate.getDate())+"' and refferPaid=0 and RefferBy = (select DoctorCode from tbdoctorinfo where name = '"+cmbDoctorName.txtMrNo.getText().toString().trim()+"') ";
				System.out.println(sql);
				ResultSet rs=db.sta.executeQuery(sql);
				while(rs.next()){
					model.addRow(new Object[]{i,
							...
							df.format(Double.parseDouble(rs.getString("TotalCharge"))),
							df.format(Double.parseDouble(rs.getString("totalDiscount"))),
							df.format(Double.parseDouble(rs.getString("TotalPayable"))),
							df.format(Double.parseDouble(rs.getString("PayableComission"))),
							new Boolean(true)});
					i++;
				}
				setTotalAmount();
//				Far
			...
			JOptionPane.showMessageDialog(null, "Warning!!,Please Select Doctor Name");
		}
	}
	...
	private void btnPostEvent(){
			if(!cmbDoctorName.txtMrNo.getText().trim().toString().isEmpty()){
					...
					if(confrim==JOptionPane.YES_OPTION){
						try {					
							for(int a=0;a<table.getRowCount();a++){
								Boolean check=(Boolean) table.getValueAt(a, 11);
								if(check){
									String UpdateReffer="update TbLabPatient set refferPaid='1' where labId='"+table.getValueAt(a, 1).toString()+"' and FiscalYear='"+table.getValueAt(a, 2).toString()+"'";
									System.out.println(UpdateReffer);
									db.sta.executeUpdate(UpdateReffer);
...
//							TotalTestAmount=TotalTestAmount+Double.parseDouble(table.getValueAt(a, 9).toString());
							TotalConsultantAmount=TotalConsultantAmount+Double.parseDouble(table.getValueAt(a, 10).toString());
						}
		}
