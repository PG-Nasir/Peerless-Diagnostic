...
public class BioChemestry extends JPanel{
	...
	String Col[]={"T.P.ID","Test Perticulars","Extra Info.","Test Result","GFR Calculate","Flag","Normal Ranges","Uom","Sorting","Test Name","Test Code"};
	Object Row[][]={};
...
/*		@Override
	...
	JRadioButton btnSerum=new JRadioButton("Serum Creatinine");
...
/*		model.addTableModelListener(new TableModelListener() {
	...
	private boolean checkSugarCalculation(String TestName){
		try {
			ResultSet rs=db.sta.executeQuery("select * from tbsubtestname where SubTestName='"+TestName+"' and CalculateType='Sugar'");
			while(rs.next()){
				return true;
			}
		} catch (Exception e) {
	...
	public void btnPrintEvent(String TestCode,String BillId,String Reg,String Name,String Age,String Month,String Day,String Sex,String Cabin,String Consultant,Date OrDate,String MainTestList,String FiscalYear){
		try {
			...
			for(int r = 0; r <table.getRowCount(); r++){
				ara[r]=Integer.parseInt(table.getValueAt(r, 8).toString());
			}
			...
			for (int r = 0; r <table.getRowCount(); r++)
			{
					...
					for(int a=0;a<table.getRowCount();a++){
						if(ara[r]==Integer.parseInt(table.getValueAt(a, 8).toString().trim())){
							ColCount=0;
							if(!table.getValueAt(r, 3).toString().trim().equals("")){
								map=new HashMap();
								...
								map.put("ExtraInfo",table.getValueAt(a, 2).toString().trim());
								map.put("Result",table.getValueAt(a, 3).toString().trim());
								map.put("CalculateValue",table.getValueAt(a, 4).toString().trim());
								map.put("Flag",table.getValueAt(a, 5).toString().trim());
								map.put("username",sessionBeam.getUserName());
								
								if(!table.getValueAt(a, 6).toString().trim().equals("")){
									normalRange=table.getValueAt(a, 5).toString().trim();
								}
								
								map.put("NormalRange",table.getValueAt(a, 6).toString().trim());
								map.put("Unit",table.getValueAt(a, 7).toString().trim());
								...
								map.put("MainTestName",table.getValueAt(a, 9).toString().trim());
			...
			if(normalRange!="1"){
				...
				String input = "";
				//input="NewFormetReport/BioChemistry3ColWithOutGroupDemo.jrxml";
				input=btnSingleTest.isSelected()?"NewFormetReport/BioChemistry3ColWithOutGroup.jrxml":btnGroupTest.isSelected()?"NewFormetReport/BioChemistry3ColWithGroup.jrxml":btnSerum.isSelected()?"NewFormetReport/BioChemistryForCreatinine.jrxml":"NewFormetReport/BioChemistry3ColWithOutGroupDiabetes.jrxml";
			...
			for(int a=0;a<table.getRowCount();a++){
				String sqlprint="update tblabreportvalue set printQty=printQty+'1' where FiscalYear='"+FiscalYear+"' and labBillId='"+BillId+"' and testCode='"+table.getValueAt(a, 9).toString().trim()+"'";
	...
	public void setPrintData(String labPid,String testCode,String TestName,String labId,Date date,String FiscalYear){
		try {
			...
			if(temp==0){
				...
				for(int a=0;a<i;a++){
					ResultSet rs2=db.sta.executeQuery("select value,ExtraInfo,CalculateValue,Flag,Sorting,(select Name from tbdoctorinfo where autoId=tblabreportvalue.LabInchargeId) as LabInchargeName,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.LabInchargeId) as LabInchargeDegree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.CheckedById) as CheckedByName,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.CheckedById) as CheckedByDegree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.Doctor1Id) as Doctor1Name,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.Doctor1Id) as Doctor1Degree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.Doctor2Id) as Doctor2Name,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.Doctor2Id) as Doctor2Degree,(select username from tblogin where user_id=tblabreportvalue.createBy) as username from tblabreportvalue where FiscalYear='"+FiscalYear+"' and labPid=2 and labBillId='"+labId+"' and rId='"+testCode+"'");
					while(rs2.next()){
						username=rs2.getString("username");
						model.addRow(new Object[]{testCode,TestList.get(a),rs2.getString("ExtraInfo"),rs2.getString("value"),rs2.getString("CalculateValue"),rs2.getString("Flag"),NormalRangeList.get(a),UnitList.get(a),rs2.getString("Sorting"),TestName,testCode});
...
/*			for(int a=table.getRowCount()-1;a>=0;a--){
			...
			if(temp==1){
				...
				for(int a=count;a<(i+count);a++){
					ResultSet rs2=db.sta.executeQuery("select ExtraInfo,value,CalculateValue,Flag,Sorting,(select Name from tbdoctorinfo where autoId=tblabreportvalue.LabInchargeId) as LabInchargeName,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.LabInchargeId) as LabInchargeDegree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.CheckedById) as CheckedByName,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.CheckedById) as CheckedByDegree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.Doctor1Id) as Doctor1Name,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.Doctor1Id) as Doctor1Degree,(select Name from tbdoctorinfo where autoId=tblabreportvalue.Doctor2Id) as Doctor2Name,(select ISNULL('#'+Degree,'#') from tbdoctorinfo where autoId=tblabreportvalue.Doctor2Id) as Doctor2Degree,(select username from tblogin where user_id=tblabreportvalue.createBy) as username from tblabreportvalue where FiscalYear='"+FiscalYear+"' and labPid=2 and labBillId='"+labId+"' and rId='"+table.getValueAt(a, 0).toString()+"'");
					while(rs2.next()){
						
						table.setValueAt(rs2.getString("ExtraInfo"), a, 2);
						table.setValueAt(rs2.getString("value"), a, 3);
						table.setValueAt(rs2.getString("CalculateValue"), a, 4);
						table.setValueAt(rs2.getString("Flag"), a, 5);
						table.setValueAt(rs2.getString("Sorting"), a, 8);
						username=rs2.getString("username");
						cmbLabIncharge.txtMrNo.setText((rs2.getString("LabInchargeName")==null?"":rs2.getString("LabInchargeName"))+(rs2.getString("LabInchargeDegree")==null?"":rs2.getString("LabInchargeDegree")));
						cmbCheckedBy.txtMrNo.setText((rs2.getString("CheckedByName")==null?"":rs2.getString("CheckedByName"))+(rs2.getString("CheckedByDegree")==null?"":rs2.getString("CheckedByDegree")));
						cmbDoctorName1.txtMrNo.setText((rs2.getString("Doctor1Name")==null?"":rs2.getString("Doctor1Name"))+(rs2.getString("Doctor1Degree")==null?"":rs2.getString("Doctor1Degree")));
	...
	public void savebtnActionEvent(String testCode,String labId,Date date,String FiscalYear){
		...
		try {				
				...
				for(int a=0;a<table.getRowCount();a++){
					if(table.getValueAt(a, 0).toString()!="" && table.getValueAt(a, 7).toString()!=""){
						
						if(checkSugarCalculation(table.getValueAt(a, 1).toString())){
							double OriginalValue=Double.parseDouble(table.getValueAt(a, 3).toString());
							double dividedValue=OriginalValue/18;
							table.setValueAt("("+df.format(dividedValue)+")", a, 4);
						}
						
							if(!checkLabReport(table.getValueAt(a, 0).toString(),labId,2,table.getValueAt(a, 10).toString(),FiscalYear)){
								AutoId();
								insertdata(a,autoId,table.getValueAt(a, 0).toString(),table.getValueAt(a, 2).toString(),table.getValueAt(a, 3).toString(),table.getValueAt(a, 4).toString(),table.getValueAt(a, 5).toString(),a,table.getValueAt(a, 10).toString(),table.getValueAt(a, 8).toString(),LabInchargeId,CheckedById,Doctor1Id,Doctor2Id,FiscalYear);
							}
							else{
								updateData(a,autoId,table.getValueAt(a, 0).toString(),table.getValueAt(a, 2).toString(),table.getValueAt(a, 3).toString(),table.getValueAt(a, 4).toString(),table.getValueAt(a, 5).toString(),a,table.getValueAt(a, 10).toString(),table.getValueAt(a, 8).toString(),LabInchargeId,CheckedById,Doctor1Id,Doctor2Id,FiscalYear);
							}
	...
	public void insertdata(int r,String autoID,String rId,String ExtraInfo,String value,String CalculateValue,String Flag,int row,String TestCode,String Sorting,String LabInchargeId,String CheckedById,String Doctor1Id,String Doctor2Id,String FiscalYear){
		try {
			noteAutoId();
			String sql="insert into tblabreportvalue (autoId,rId,ExtraInfo,value,CalculateValue,Flag,labPId,labBillId,testCode,Sorting,LabInchargeId,CheckedById,Doctor1Id,Doctor2Id,FiscalYear,date,entryTime,createBy) values ('"+autoID+"',"
					+ "'"+rId+"',"
					+ "'"+ExtraInfo+"',"
					+ "'"+value+"',"
					+ "'"+CalculateValue+"',"
	...
	public void updateData(int r,String autoID,String rId,String ExtraInfo,String value,String CalculateValue,String Flag,int row,String TestCode,String Sorting,String LabInchargeId,String CheckedById,String Doctor1Id,String Doctor2Id,String FiscalYear){
		try {
			String sql="update tblabreportvalue set ExtraInfo='"+ExtraInfo+"',value='"+value+"',CalculateValue='"+CalculateValue+"',Flag='"+Flag+"',Sorting='"+Sorting+"',LabInchargeId='"+LabInchargeId+"',CheckedById='"+CheckedById+"',Doctor1Id='"+Doctor1Id+"',Doctor2Id='"+Doctor2Id+"',createBy='"+sessionBeam.getUserId()+"' where FiscalYear='"+FiscalYear+"' and rId='"+rId+"' and labBillId='"+labbillId+"' and testCode='"+TestCode+"' ";
			System.out.println(sql);
			db.sta.executeUpdate(sql);
		} catch (Exception e) {
			e.printStackTrace();
