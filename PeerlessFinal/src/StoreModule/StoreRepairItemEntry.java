...
public class StoreRepairItemEntry extends JPanel{
	...
	BufferedImage image;
	public StoreRepairItemEntry(SessionBeam sessionbeam) {
	...
	public void getInvoiceData(){
		try {
			...
			String sql="select *from TbStoreTransectionDetails where invoiceNo='"+sessionbeam.getInvoiceId()+"' && type=6";
			System.out.println(sql);
			...
			ResultSet rs=db.sta.executeQuery("select TbStoreTransectionInvoice.invoiceNo,"
					...
					+ " from TbStoreTransectionInvoice where TbStoreTransectionInvoice.invoiceNo='"+sessionbeam.getInvoiceId()+"' && TbStoreTransectionInvoice.type=6");
			while(rs.next()){
		...
		try {
			...
			if(!txtDuePaymentAmount.getText().trim().toString().isEmpty()){
				...
				if(confrim==JOptionPane.YES_NO_OPTION){
					...
					String sql="update TbStoreTransectionInvoice set cash=cash+'"+cashAmount+"',card=card+'"+cardAmount+"',paid=paid+'"+paid+"', date=CURRENT_TIMESTAMP,entryTime=CURRENT_TIMESTAMP,createBy='"+sessionbeam.getUserId()+"' where invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"' and type=6";
					System.out.println(sql);
					db.sta.executeUpdate(sql);
	...
	private void btnActionEvent(){
			...
			public void keyPressed(KeyEvent e) {
				...
				if(e.getKeyCode()==KeyEvent.VK_ENTER){
					try {
						...
						String sql="select *from TbStoreTransectionDetails where invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"' and type=6";
						System.out.println(sql);
						ResultSet rs1=db.sta.executeQuery(sql);
						while(rs1.next()){
						...
						String sql1="select TbStoreTransectionInvoice.invoiceNo,"
								...
								+ "TbStoreTransectionInvoice.paid,"
								+ "TbStoreTransectionInvoice.date"
								+ " from TbStoreTransectionInvoice where TbStoreTransectionInvoice.invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"' and TbStoreTransectionInvoice.type=6";
						...
						ResultSet rs2=db.sta.executeQuery("select *from TbStoreTransectionInvoice where invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"' and type=6");
						while(rs2.next()){
		...
		btnRefresh.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				autoId();
				try {
					db.sta.executeUpdate("delete from TbStoreTransectionDetails where type='6' and invoiceNo IS NULL");
	...
	private boolean UpInvoicedoplicateName(){
		try {
			ResultSet rs=db.sta.executeQuery("select productName,buyPrice from TbStoreTransectionDetails where type='6' and invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"'");
			while(rs.next()){
	...
	private boolean InvoicedoplicateName(){
		try {
			ResultSet rs=db.sta.executeQuery("select productName,buyPrice from TbStoreTransectionDetails where type='6' and invoiceNo IS NULL");
			while(rs.next()){
				if(cmbProductName.txtMrNo.getText().toString().equalsIgnoreCase(rs.getString("productName"))){
					return true;
				}
		...
		try {
			if(findId==1){
				if(! UpInvoicedoplicateName()){
					...
					qty=Double.parseDouble(txtQty.getText().trim().toString());
					price=Double.parseDouble(txtPrice.getText().trim().toString());
					ResultSet rs=db.sta.executeQuery("select qty from TbStoreTransectionDetails where productName='"+cmbProductName.txtMrNo.getText().toString()+"' and type='6' and invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"'");
					while(rs.next()){
						qty=qty+Double.parseDouble(rs.getString("qty"));
					}
					totalPrice=qty*price;
					String query="update TbStoreTransectionDetails set qty='"+qty+"',buyPrice='"+price+"',totalPrice='"+df.format(totalPrice)+"' where productName='"+cmbProductName.txtMrNo.getText().toString()+"' and type='6' and invoiceNo='"+txtInvoiceNo.getText().trim().toString()+"'";
				...
				if(!InvoicedoplicateName()){
					...
					ResultSet rs=db.sta.executeQuery("select qty from TbStoreTransectionDetails where productName='"+cmbProductName.txtMrNo.getText().toString()+"' and type='6' and invoiceNo IS NULL");
					while(rs.next()){
						qty=qty+Double.parseDouble(rs.getString("qty"));
					}
					totalPrice=qty*price;
					String query="update TbStoreTransectionDetails set qty='"+qty+"',buyPrice='"+price+"',totalPrice='"+df.format(totalPrice)+"' where productName='"+cmbProductName.txtMrNo.getText().toString()+"' and type='6' and invoiceNo IS NULL";
					System.out.println(query);
					db.sta.executeUpdate(query);
					JOptionPane.showMessageDialog(null, "Product Order Successfully Accept");
					tableValue();
	...
	public void UpdateInvCastTransection(){
		try {
			...
			String invoiceUpdate="update TbStoreTransectionInvoice set cashId='"+tranId+"' where invoiceNo='"+txtInvoiceNo.getText().toString()+"' && type=6";
			System.out.println(invoiceUpdate);
			db.sta.executeUpdate(invoiceUpdate);
		} catch (Exception e) {
			e.printStackTrace();
	...
	public void autoinvoiceId(){
		try {
			String sql="select (ISNULL(max(invoiceNo),0)+1)as invoiceNo from TbStoreTransectionInvoice where type=6 ";
			ResultSet rs=db.sta.executeQuery(sql);
			while(rs.next())
			{
	...
	public void tableValue(){
		try {
			...
			ResultSet rs=db.sta.executeQuery("select *from TbStoreTransectionDetails where invoiceNo IS NULL and type=6");
			while(rs.next()){
	...
	private void UpdateInvoieValue(){
		try {
			...
			ResultSet rs=db.sta.executeQuery("select *from TbStoreTransectionDetails where (invoiceNo='"+txtInvoiceNo.getText().toString()+"' or invoiceNo IS NULL) and type=6");
			while(rs.next()){
